...
async function checkPost() {
  try {
    ...
    const foodReview = await client.fetch(
      `*[_type == "foodReview" && title match "*MatCHA Chai Brunch*"][0]{ _id, title, _type, date }`,
    )
    console.log('Food Review (new structure):', foodReview)

    // Check if it exists as legacy post
    const legacyPost = await client.fetch(
      `*[_type == "post" && title match "*MatCHA Chai Brunch*"][0]{ _id, title, _type, linkType, date }`,
    )
