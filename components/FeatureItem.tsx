...
const FeatureItem: React.FC<FeatureItemProps> = ({ label, isAvailable, details }) => {
  ...
  return (
    <div className="flex items-baseline"> {/* Use items-baseline for better alignment */}
      ...
      {/* Label and Details */}
      <p className="text-sm leading-relaxed text-gray-600"> {/* Adjusted text color */}
        {label}
        {isAvailable && details && ` (${details})`} {/* Show details only if available */}
      </p>
    </div>
  );
};
