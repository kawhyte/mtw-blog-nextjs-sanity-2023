  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (
        ...
        !picksDropdownRef.current.contains(event.target as Node)
      ) {
        setIsPicksDropdownOpen(false)
      }
    }
    ...
    if (isPicksDropdownOpen && window.innerWidth >= 768) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full bg-background shadow-sm">
      ...
      <div className="mx-auto flex max-w-396 items-center justify-between px-4 py-3">
        ...
        <form
          onSubmit={handleSearchSubmit}
          className="ml-4 hidden flex-1 max-w-md md:ml-6 md:block"
        >
        ...
        <div className="hidden items-center space-x-1 md:flex lg:space-x-4">
          <NavItem
            href="/arenas"
            icon={<LiaBasketballBallSolid size={22} />}
            text="NBA/WNBA Arenas"
          />
            ...
            <button // Using button as it triggers a UI change, not navigation
              ...
              className={cn(
                ...
                // Add active state based on if a child route is active or dropdown is open
                {
                  'text-primary':
                    isPicksDropdownOpen ||
                    router.pathname.startsWith('/top-hotel-picks') ||
      ...
      >
        <div className="flex flex-col p-4">
          ...
          <MobileNavItem
            href="/arenas"
            icon={<LiaBasketballBallSolid size={18} />}
            text="NBA & WNBA Arenas"
            onClick={toggleMenu}
          />
          ...
          {isPicksDropdownOpen && (
            <div
              id="mobile-picks-submenu"
              className="ml-4 flex flex-col border-l border-border pl-2"
            >
              {' '}
              {/* Indent sub-items */}
              {/* ***** CORRECTION ***** */}
...
function NavItem({
  ...
  return (
    ...
    >
      <span className={cn('mb-1', isActive ? 'text-primary' : 'text-primary')}>
        {icon}
      </span>
      <span>{text}</span>
