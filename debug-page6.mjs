...
async function debugPagination() {
  try {
    ...
    for (let page = 1; page <= Math.min(totalPages, 7); page++) {
      ...
      const pageData =
        await client.fetch(`*[_type == "hotelReview"] | order(date desc) [${start}...${end}] {
        _id,
        title,
        date,
        coverImage,
      ...
      pageData.forEach((item, index) => {
        ...
        console.log(
          `  ${start + index + 1}. ${item.title?.substring(0, 40)}${item.title?.length > 40 ? '...' : ''} | Image: ${hasImage ? '✓' : '✗'} | Rating: ${hasRating ? '✓' : '✗'} | Long: ${hasLongTitle ? '✓' : '✗'}`,
        )
      })
    }
  } catch (error) {
