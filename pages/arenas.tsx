...
export default function Page(props: PageProps) {
  ...
  const [filterCriteria, setFilterCriteria] =
    useState<FilterCriteriaType>('all')
    ...
    const sortedForDisplay = [...filtered].sort((a, b) => {
      ...
      switch (sortCriteria) {
        ...
        case 'highest': {
          ...
          const { average: scoreA } = calculateAverageRating(
            a.arenaReview || {},
          )
          const { average: scoreB } = calculateAverageRating(
            b.arenaReview || {},
          )
          const numScoreA = parseFloat(scoreA) || 0
          const numScoreB = parseFloat(scoreB) || 0
      ...
      <Layout preview={preview} loading={false}>
        ...
        <div className="container mx-auto mb-12 mt-6 flex max-w-4xl flex-col items-center justify-between gap-y-4 rounded-lg bg-gray-50 px-4 py-4 shadow-sm md:flex-row md:gap-y-0 md:px-6">
          ...
          <div className="flex w-full items-center justify-center space-x-2 md:w-auto md:justify-end">
            ...
            {(['all', 'visited', 'notVisited'] as FilterCriteriaType[]).map(
              (criteria) => (
                <button
                  key={criteria}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-4 gap-y-8 px-4 md:gap-x-6 md:gap-y-10 md:px-6 lg:gap-x-8 mx-auto container">
          {arenasToDisplay.length > 0 ? (
            ...
            arenasToDisplay.map((item) => {
              // Loop through the list sorted for display
              ...
              const rankNumber = item.visited
                ? (ratingRankMap.get(item._id) ?? null)
                : null

              // Check slug for linking
              if (!item.slug) {
                console.warn(
                  `Arena "${item.name || item._id}" is missing a slug. Skipping link.`,
                )
