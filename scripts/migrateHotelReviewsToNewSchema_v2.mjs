...
async function migrateHotelReviews() {
  try {
    ...
    if (references.length > 0) {
      console.log(
        `⚠️ Found ${references.length} documents referencing hotel posts:`,
      )
      references.slice(0, 5).forEach((ref) => {
        console.log(
          `  - ${ref._type}: "${ref.title || ref._id}" references ${ref.referencedPosts?.length || 0} hotel posts`,
        )
      })
    ...
    for (const hotelReview of hotelReviews) {
      try {
        ...
        console.error(
          `❌ Failed to create: ${hotelReview.title}`,
          error.message,
        )
      }
    }
    ...
    console.log(`📊 Results:`)
    console.log(
      `  ✅ Created: ${createdDocuments.length}/${hotelReviews.length} hotel review documents`,
    )

    if (references.length > 0) {
      console.log(
        `  ⚠️ Kept: ${backupData.length} old hotel post documents (due to references)`,
      )
      console.log('\n📋 Next Steps:')
      console.log(
        '  1. Update any references to point to new hotel review documents',
      )
      console.log(
        '  2. Manually delete old hotel posts when no longer referenced',
      )
